...
from groot.rl.envs.base_task.base_task import BaseTask
...
class LocoManipBase(BaseTask):
    def __init__(self, config, device):
        ...
        self.init_root_states = copy.deepcopy(self.simulator.all_root_states)
    ...
    def reset_all(self):
        ...
        self.simulator.set_task_root_state_tensor(
            torch.arange(self.num_envs, device=self.device), self.init_root_states
        )
    ...
    def _post_physics_step(self):
        ...
        if len(refresh_env_ids) > 0:
            ...
            self.simulator.set_task_root_state_tensor(refresh_env_ids, self.init_root_states)
    ...
    def _update_reset_buf(self):
        ...
        if self.config.termination.terminate_by_hand_object_contact:
            termination_contact_force = self.config.termination_scales.get(
                "termination_contact_force", 1.0
            )
            self.reset_buf |= torch.any(
    ...
    def _load_z_model(
        ...
        from groot.rl.utils.helpers import pre_process_config
