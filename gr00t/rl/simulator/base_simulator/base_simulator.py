...
class BaseSimulator:
    ...
    def load_assets(self, robot_config):
        ...
        Args:
            robot_config (dict): rl Configuration for the robot asset.
        """
    ...
    def set_task_root_state_tensor(self, set_env_ids, root_states):
        ...
        """
        raise NotImplementedError(
            "The 'set_task_root_state_tensor' method must be implemented in subclasses."
        )
    ...
    def set_task_dof_state_tensor(self, set_env_ids, dof_states):
        ...
        """
        raise NotImplementedError(
            "The 'set_task_dof_state_tensor' method must be implemented in subclasses."
        )
    ...
    def trigger_reset_events(
        ...
        """
        raise NotImplementedError("The 'reset' method must be implemented in subclasses.")

    def get_3d_force_sensor_reading(self, env_id=None):
        """
        Get 3D force sensor readings for a specific environment.

        Args:
            env_id (int): Environment ID. If None, returns readings for all environments.

        Returns:
            torch.Tensor: Force sensor readings tensor.
        """
        raise NotImplementedError(
            "The 'get_3d_force_sensor_reading' method must be implemented in subclasses."
        )

    def get_6d_force_sensor_reading(self, env_id=None):
        """
        Get 6D force sensor readings for a specific environment.
        ...
        """
        raise NotImplementedError(
            "The 'get_6d_force_sensor_reading' method must be implemented in subclasses."
        )

    def apply_rigid_body_force_at_pos_tensor(self, force_tensor, pos_tensor):
        """
        Applies a force at a specified position on the rigid body.

        Args:
            force_tensor (torch.Tensor): Tensor containing the force to apply.
            pos_tensor (torch.Tensor): Tensor containing the position where the force is applied.
        """
        raise NotImplementedError(
            "The 'apply_rigid_body_force_at_pos_tensor' method must be implemented in subclasses."
        )
