...
class Terrain:
    ...
    def randomized_terrain(self):
        ...
        for k in range(self.cfg.num_sub_terrains):
            print(
                "generating randomized terrains %d / %d     " % (k, self.cfg.num_sub_terrains),
                end="\r",
            )
    ...
    def gap_terrain_func(self, terrain, difficulty):
        ...
        terrain.height_field_raw[center_x - x2 : center_x + x2, center_y - y2 : center_y + y2] = (
            -1000
        )
    ...
    def rough_terrain_func(self, terrain, difficulty):
        ...
        terrain.height_field_raw = (
            np.random.uniform(-max_height * 2 - 0.025, -0.025, terrain.height_field_raw.shape)
    ...
    def maze_terrain_func(self, terrain, difficulty):
        ...
        for _col in range(0, terrain.length, path_width):
            ...
            terrain.height_field_raw[
                _col : _col + path_width, center_y + y_low[_col] : center_y + y_high[_col]
            ] = 0.0
        terrain.height_field_raw[:room_size, center_y - room_size : center_y + room_size] = 0.0
        terrain.height_field_raw[-room_size:, center_y - room_size : center_y + room_size] = 0.0
